...
function Challenges({
  ...
  // Run code using refs to always get latest values
  const runCode = useCallback(
    (withTests = false) => {
      ...
      try {
        ...
        if (withTests && challenge.testCases.length > 0) {
          runTestCases(currentCode, challenge);
        } else if (withTests && challenge.testCases.length === 0) {
          setOutput((prev) => [
            ...prev,
            {
              type: "info",
              message: "ℹ No test cases available for this challenge.",
            },
          ]);
        } else {
          setOutput((prev) => [
    ...
    [showToast, set.id, onSolved]
  );

  const runTestCases = (currentCode, challenge) => {
    let passed = 0;
    ...
    if (passed === total && total > 0) {
      onSolved(set.id, challenge.id);
    }
  };

  // Keyboard handler for Monaco
  const handleEditorKeyDown = useCallback(
    (e) => {
      if (e.shiftKey) {
        runCode(true); // Run with tests
      } else {
        runCode(false); // Run without tests
      }
    },
    [runCode]
  );
      ...
      <aside className="challenge-panel">
        <div className="panel-header-row">
          ...
          <h2 className="panel-title">
            <span className="glitch" data-text="CHALLENGES">
              CHALLENGES
            </span>
          </h2>
        </div>
            ...
            filteredChallenges.map((challenge, index) => (
              <li
                ...
                className={`challenge-item ${
                  currentChallenge?.id === challenge.id ? "active" : ""
                } ${isChallengeSolved(set.id, challenge.id) ? "solved" : ""}`}
      ...
      <section className="workspace">
        ...
        <div
          className={`collapsible-section ${
            problemExpanded ? "section-expanded" : "section-collapsed"
          }`}
        >
          <div
          ...
          >
            ...
            <span className="problem-name">
              {currentChallenge
                ? `${currentChallenge.id}. ${currentChallenge.name}`
                : "Select a Challenge"}
            </span>
          </div>
          <div
            className={`section-content ${problemExpanded ? "expanded" : ""}`}
          >
            <div className="problem-description">
              {currentChallenge ? (
                ...
                <p>
                  👈 Pick a challenge from the left panel to begin your DSA
                  journey!
                </p>
              )}
            </div>
        ...
        <div
          className={`collapsible-section ${
            solutionExpanded ? "section-expanded" : "section-collapsed"
          }`}
        >
          <div
          ...
          <div
            className={`section-content ${solutionExpanded ? "expanded" : ""}`}
          >
        ...
        <div
          className={`collapsible-section ${
            outputExpanded ? "section-expanded" : "section-collapsed"
          }`}
        >
          <div
          ...
          >
            ...
            <div className="output-actions">
              ...
              <button
                className="action-btn view-tests-btn"
                onClick={onViewTests}
              >
                👁 VIEW TESTS
              </button>
          ...
          <div
            className={`section-content ${outputExpanded ? "expanded" : ""}`}
          >
